...
const AP_RangeFinder_VL53L0X::RegData AP_RangeFinder_VL53L0X::tuning_data[] = {
        ...
        { 0xFF, 0x00 }, { 0x30, 0x09 }, { 0x54, 0x00 }, { 0x31, 0x04 }, { 0x32,
                0x03 }, { 0x40, 0x83 }, { 0x46, 0x25 }, { 0x60, 0x00 }, { 0x27,
                0x00 }, { 0x50, 0x06 }, { 0x51, 0x00 }, { 0x52, 0x96 }, { 0x56,
                0x08 }, { 0x57, 0x30 }, { 0x61, 0x00 }, { 0x62, 0x00 }, { 0x64,
                0x00 }, { 0x65, 0x00 }, { 0x66, 0xA0 },

        { 0xFF, 0x01 }, { 0x22, 0x32 }, { 0x47, 0x14 }, { 0x49, 0xFF }, { 0x4A,
                0x00 },

        { 0xFF, 0x00 }, { 0x7A, 0x0A }, { 0x7B, 0x00 }, { 0x78, 0x21 },

        { 0xFF, 0x01 }, { 0x23, 0x34 }, { 0x42, 0x00 }, { 0x44, 0xFF }, { 0x45,
                0x26 }, { 0x46, 0x05 }, { 0x40, 0x40 }, { 0x0E, 0x06 }, { 0x20,
                0x1A }, { 0x43, 0x40 },

        { 0xFF, 0x00 }, { 0x34, 0x03 }, { 0x35, 0x44 },

        { 0xFF, 0x01 }, { 0x31, 0x04 }, { 0x4B, 0x09 }, { 0x4C, 0x05 }, { 0x4D,
                0x04 },

        { 0xFF, 0x00 }, { 0x44, 0x00 }, { 0x45, 0x20 }, { 0x47, 0x08 }, { 0x48,
                0x28 }, { 0x67, 0x00 }, { 0x70, 0x04 }, { 0x71, 0x01 }, { 0x72,
                0xFE }, { 0x76, 0x00 }, { 0x77, 0x00 },

        { 0xFF, 0x01 }, { 0x0D, 0x01 },

        { 0xFF, 0x00 }, { 0x80, 0x01 }, { 0x01, 0xF8 },

        { 0xFF, 0x01 }, { 0x8E, 0x01 }, { 0x00, 0x01 }, { 0xFF, 0x00 }, { 0x80,
                0x00 }, };
 ...
 */
AP_RangeFinder_VL53L0X::AP_RangeFinder_VL53L0X(RangeFinder &_ranger,
        uint8_t instance, RangeFinder::RangeFinder_State &_state,
        AP_HAL::OwnPtr<AP_HAL::I2CDevice> _dev) :
        AP_RangeFinder_Backend(_ranger, instance, _state,
                MAV_DISTANCE_SENSOR_LASER), dev(std::move(_dev)) {
}
...
/*
 ...
 there.
 */
AP_RangeFinder_Backend* AP_RangeFinder_VL53L0X::detect(RangeFinder &_ranger,
        uint8_t instance, RangeFinder::RangeFinder_State &_state,
        AP_HAL::OwnPtr<AP_HAL::I2CDevice> dev) {
    AP_RangeFinder_VL53L0X *sensor = new AP_RangeFinder_VL53L0X(_ranger,
            instance, _state, std::move(dev));
...
bool AP_RangeFinder_VL53L0X::check_id(void) {
    uint8_t v1, v2;
    if (!dev->read_registers(0xC0, &v1, 1) || !dev->read_registers(0xC1, &v2, 1)
            || v1 != 0xEE || v2 != 0xAA) {
        return false;
    }
...
void AP_RangeFinder_VL53L0X::getSequenceStepEnables(
...
uint32_t AP_RangeFinder_VL53L0X::timeoutMclksToMicroseconds(
...
void AP_RangeFinder_VL53L0X::getSequenceStepTimeouts(
    ...
    timeouts->pre_range_mclks = decodeTimeout(
            read_register16(PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI));
    ...
    timeouts->final_range_mclks = decodeTimeout(
            read_register16(FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI));
...
uint32_t AP_RangeFinder_VL53L0X::timeoutMicrosecondsToMclks(
...
void AP_RangeFinder_VL53L0X::init() {
    ...
    write_register(MSRC_CONFIG_CONTROL,
            read_register(MSRC_CONFIG_CONTROL) | 0x12);

    // set final range signal rate limit to 0.25 MCPS (million counts per second)
    write_register16(FINAL_RANGE_CONFIG_MIN_COUNT_RATE_RTN_LIMIT,
            uint16_t(0.25 * (1 << 7)));
    ...
    if (!dev->read_registers(GLOBAL_CONFIG_SPAD_ENABLES_REF_0, ref_spad_map,
            6)) {
    ...
    write_register(GPIO_HV_MUX_ACTIVE_HIGH,
            read_register(GPIO_HV_MUX_ACTIVE_HIGH) & ~0x10); // active low
...
bool AP_RangeFinder_VL53L0X::performSingleRefCalibration(
