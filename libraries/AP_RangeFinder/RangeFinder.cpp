...
void RangeFinder::detect_instance(uint8_t instance) {
    ...
    case RangeFinder_TYPE_PLI2CV3:
        if (!_add_backend(
                AP_RangeFinder_PulsedLightLRF::detect(1, *this, instance,
                        state[instance], type))) {
            _add_backend(
                    AP_RangeFinder_PulsedLightLRF::detect(0, *this, instance,
                            state[instance], type));
        }
        break;
    case RangeFinder_TYPE_MBI2C:
        _add_backend(
                AP_RangeFinder_MaxsonarI2CXL::detect(*this, instance,
                        state[instance]));
        break;
    case RangeFinder_TYPE_LWI2C:
        if (_address[instance]) {
            _add_backend(
                    AP_RangeFinder_LightWareI2C::detect(*this, instance,
                            state[instance],
    ...
    case RangeFinder_TYPE_TRONE:
        if (!_add_backend(
                AP_RangeFinder_trone::detect(0, *this, instance,
                        state[instance]))) {
            _add_backend(
                    AP_RangeFinder_trone::detect(1, *this, instance,
                            state[instance]));
        }
        break;
    case RangeFinder_TYPE_VL53L0X:
        if (!_add_backend(
                AP_RangeFinder_VL53L0X::detect(*this, instance, state[instance],
                        hal.i2c_mgr->get_device(1, 0x29)))) {
            _add_backend(
                    AP_RangeFinder_VL53L0X::detect(*this, instance,
                            state[instance], hal.i2c_mgr->get_device(0, 0x29)));
        }
        break;
    ...
    case RangeFinder_TYPE_LWSER:
        ...
        if (AP_RangeFinder_LightWareSerial::detect(*this, instance,
                serial_manager)) {
            state[instance].instance = instance;
            drivers[instance] = new AP_RangeFinder_LightWareSerial(*this,
                    instance, state[instance], serial_manager);
        }
        break;
    case RangeFinder_TYPE_LEDDARONE:
        if (AP_RangeFinder_LeddarOne::detect(*this, instance, serial_manager)) {
            state[instance].instance = instance;
            drivers[instance] = new AP_RangeFinder_LeddarOne(*this, instance,
                    state[instance], serial_manager);
        }
        break;
    case RangeFinder_TYPE_ULANDING:
        if (AP_RangeFinder_uLanding::detect(*this, instance, serial_manager)) {
            state[instance].instance = instance;
            drivers[instance] = new AP_RangeFinder_uLanding(*this, instance,
                    state[instance], serial_manager);
        }
        break;
    ...
    case RangeFinder_TYPE_MAVLink:
        if (AP_RangeFinder_MAVLink::detect(*this, instance)) {
            state[instance].instance = instance;
            drivers[instance] = new AP_RangeFinder_MAVLink(*this, instance,
                    state[instance]);
        }
        break;
    case RangeFinder_TYPE_MBSER:
        if (AP_RangeFinder_MaxsonarSerialLV::detect(*this, instance,
                serial_manager)) {
            state[instance].instance = instance;
            drivers[instance] = new AP_RangeFinder_MaxsonarSerialLV(*this,
                    instance, state[instance], serial_manager);
        }
        break;
...
RangeFinder::RangeFinder_Status RangeFinder::status(uint8_t instance) const {
    ...
    if (drivers[instance] == nullptr
            || _type[instance] == RangeFinder_TYPE_NONE) {
        return RangeFinder_NotConnected;
    }
...
RangeFinder::RangeFinder_Status RangeFinder::status_orient(
...
bool RangeFinder::find_instance(enum Rotation orientation,
...
int16_t RangeFinder::ground_clearance_cm_orient(
        enum Rotation orientation) const {
    uint8_t i;
    if (find_instance(orientation, i)) {
...
bool RangeFinder::has_data(uint8_t instance) const {
    ...
    return ((state[instance].status != RangeFinder_NotConnected)
            && (state[instance].status != RangeFinder_NoData));
}
...
void RangeFinder::update_pre_arm_check(uint8_t instance) {
    ...
    if (state[instance].pre_arm_check
            || state[instance].status == RangeFinder_NotConnected
            || state[instance].status == RangeFinder_NoData) {
        return;
    }

    // update min, max captured distances
    state[instance].pre_arm_distance_min = MIN(state[instance].distance_cm,
            state[instance].pre_arm_distance_min);
    state[instance].pre_arm_distance_max = MAX(state[instance].distance_cm,
            state[instance].pre_arm_distance_max);

    // Check that the range finder has been exercised through a realistic range of movement
    if (((state[instance].pre_arm_distance_max
            - state[instance].pre_arm_distance_min)
            > RANGEFINDER_PREARM_REQUIRED_CHANGE_CM)
            && (state[instance].pre_arm_distance_max
                    < RANGEFINDER_PREARM_ALT_MAX_CM)
            && ((int16_t) state[instance].pre_arm_distance_min
                    < (MAX(_ground_clearance_cm[instance],
                            min_distance_cm(instance)) + 10))
            && ((int16_t) state[instance].pre_arm_distance_min
                    > (MIN(_ground_clearance_cm[instance],
                            min_distance_cm(instance)) - 10))) {
        state[instance].pre_arm_check = true;
    }
}

const Vector3f& RangeFinder::get_pos_offset_orient(
        enum Rotation orientation) const {
    uint8_t i = 0;
    if (find_instance(orientation, i)) {
...
MAV_DISTANCE_SENSOR RangeFinder::get_sensor_type_orient(
        enum Rotation orientation) const {
    uint8_t i;
    if (find_instance(orientation, i)) {
