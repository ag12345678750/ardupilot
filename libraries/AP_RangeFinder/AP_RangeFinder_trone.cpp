 ...
 */
AP_RangeFinder_trone::AP_RangeFinder_trone(uint8_t bus, RangeFinder &_ranger,
        uint8_t instance, RangeFinder::RangeFinder_State &_state) :
        AP_RangeFinder_Backend(_ranger, instance, _state,
                MAV_DISTANCE_SENSOR_LASER), dev(
                hal.i2c_mgr->get_device(bus, TRONE_I2C_ADDR)) {
}
...
AP_RangeFinder_Backend* AP_RangeFinder_trone::detect(uint8_t bus,
    ...
    AP_RangeFinder_trone *sensor = new AP_RangeFinder_trone(bus, _ranger,
            instance, _state);
    if (!sensor) {
        return nullptr;
    }
