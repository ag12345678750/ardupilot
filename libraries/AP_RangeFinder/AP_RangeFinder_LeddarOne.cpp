 ...
 */
AP_RangeFinder_LeddarOne::AP_RangeFinder_LeddarOne(RangeFinder &_ranger,
...
/*
 ...
 there.
 */
bool AP_RangeFinder_LeddarOne::detect(RangeFinder &_ranger, uint8_t instance,
        AP_SerialManager &serial_manager) {
...
bool AP_RangeFinder_LeddarOne::get_reading(uint16_t &reading_cm) {
    ...
    case LEDDARONE_MODBUS_STATE_AVAILABLE:
        ...
        // if status is not reading buffer, reset mod_bus status to read new buffer
                ...
                || AP_HAL::millis() - last_available_ms > 200) {
            // if read_len is zero, send request without initialize
            modbus_status =
                    (read_len == 0) ?
                            LEDDARONE_MODBUS_STATE_PRE_SEND_REQUEST :
 ...
 */
bool AP_RangeFinder_LeddarOne::CRC16(uint8_t *aBuffer, uint8_t aLength,
        bool aCheck) {
    uint16_t crc = 0xFFFF;
...
/*
 ...
 -----------------------------------------------
 */
LeddarOne_Status AP_RangeFinder_LeddarOne::parse_response(
        uint8_t &number_detections) {
    uint8_t index;
    ...
    if (read_len != LEDDARONE_READ_BUFFER_SIZE
            || read_buffer[1] != LEDDARONE_MODOBUS_FUNCTION_CODE) {
        return LEDDARONE_STATE_ERR_BAD_RESPONSE;
    }
    ...
    for (index = 0; index < number_detections; index++) {
        ...
        detections[index] = (static_cast<uint16_t>(read_buffer[index_offset])
                * 256 + read_buffer[index_offset + 1]) / 10;
