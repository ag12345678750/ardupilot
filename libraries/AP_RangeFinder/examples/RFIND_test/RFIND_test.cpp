...
void setup() {
    ...
    AP_Param::set_object_value(&sonar, sonar.var_info, "_TYPE",
...
void loop() {
    ...
    hal.console->printf(
            "All: device_0 type %d status %d distance_cm %d, device_1 type %d status %d distance_cm %d\n",
            (int) sonar._type[0], (int) sonar.status(0), sonar.distance_cm(0),
            (int) sonar._type[1], (int) sonar.status(1), sonar.distance_cm(1));
