...
void Plane::update_is_flying_5Hz(void) {
    ...
    uint32_t ground_speed_thresh_cm =
            (aparm.min_gndspeed_cm > 0) ?
                    ((uint32_t) (aparm.min_gndspeed_cm * 0.9f)) :
                    GPS_IS_FLYING_SPEED_CMS;
    bool gps_confirmed_movement = (gps.status() >= AP_GPS::GPS_OK_FIX_3D)
            && (gps.ground_speed_cm() >= ground_speed_thresh_cm);

    // airspeed at least 75% of stall speed?
    bool airspeed_movement = ahrs.airspeed_estimate(&aspeed)
            && (aspeed >= (aparm.airspeed_min * 0.75f));
    ...
    if (quadplane.is_flying()) {
        ...
        if (control_mode == AUTO) {
            ...
            if (g.crash_accel_threshold == 0) {
            ...
            } else if (ins.get_accel_peak_hold_neg_x()
                    < -(g.crash_accel_threshold)) {
                ...
                if (isFlyingProbability > 0.2f) {
                    ...
                    && (now_ms - crash_state.impact_timer_ms
                            > IS_FLYING_IMPACT_TIMER_MS)) {
    ...
    if (!crash_state.impact_detected || !is_flying_bool) {
        ...
        isFlyingProbability = (0.85f * isFlyingProbability)
                + (0.15f * (float) is_flying_bool);
    }
...
void Plane::crash_detection_update(void) {
    ...
    if (!is_flying() && arming.is_armed()) {
        if (landing.is_expecting_impact()) {
            ...
            if (!crash_state.checkedHardLanding
                    && // only check once
                    been_auto_flying
                    && (labs(ahrs.roll_sensor) > 6000
                            || labs(ahrs.pitch_sensor) > 6000)) {
                crashed = true;
                crash_state.debounce_time_total_ms = CRASH_DETECTION_DELAY_MS;

                // did we "crash" within 75m of the landing location? Probably just a hard landing
                crashed_near_land_waypoint = get_distance(current_loc,
                        mission.get_current_nav_cmd().content.location) < 75;
            ...
            switch (flight_stage) {
            case AP_Vehicle::FixedWing::FLIGHT_TAKEOFF:
                if (g.takeoff_throttle_min_accel > 0 && !throttle_suppressed) {
                    ...
                    crashed = true;
                    crash_state.debounce_time_total_ms =
                            CRASH_DETECTION_DELAY_MS;
                }
    ...
    } else if ((now_ms - crash_state.debounce_timer_ms
            >= crash_state.debounce_time_total_ms) && !crash_state.is_crashed) {
        crash_state.is_crashed = true;

        if (aparm.crash_detection_enable
                == CRASH_DETECT_ACTION_BITMASK_DISABLED) {
            if (crashed_near_land_waypoint) {
                gcs().send_text(MAV_SEVERITY_CRITICAL,
                        "Hard landing detected. No action taken");
            } else {
                gcs().send_text(MAV_SEVERITY_EMERGENCY,
                        "Crash detected. No action taken");
            }
        } else {
            if (aparm.crash_detection_enable
                    & CRASH_DETECT_ACTION_BITMASK_DISARM) {
                disarm_motors();
            }
            if (crashed_near_land_waypoint) {
...
bool Plane::in_preLaunch_flight_stage(void) {
    return (control_mode == AUTO && throttle_suppressed
