...
void Plane::adjust_altitude_target() {
    ...
    if (landing.is_flaring()) {
        ...
        landing.adjust_landing_slope_for_rangefinder_bump(rangefinder_state,
                prev_WP_loc, next_WP_loc, current_loc, auto_state.wp_distance,
                target_altitude.offset_cm);
...
void Plane::setup_glide_slope(void) {
    ...
    case AUTO:
        ...
        // obstacles.
        if (adjusted_relative_altitude_cm() > 2000
                || above_location_current(next_WP_loc)) {
            set_offset_altitude_location(next_WP_loc);
        } else {
            reset_offset_altitude();
        }
 ...
 */
void Plane::set_target_altitude_proportion(const Location &loc,
        float proportion) {
 ...
 */
void Plane::constrain_target_altitude_location(const Location &loc1,
        const Location &loc2) {
    if (loc1.alt > loc2.alt) {
        target_altitude.amsl_cm = constrain_int32(target_altitude.amsl_cm,
                loc2.alt, loc1.alt);
    } else {
        target_altitude.amsl_cm = constrain_int32(target_altitude.amsl_cm,
                loc1.alt, loc2.alt);
...
void Plane::set_offset_altitude_location(const Location &loc) {
    ...
    if (flight_stage != AP_Vehicle::FixedWing::FLIGHT_LAND) {
        ...
        if (g.glide_slope_min <= 0
                || labs(target_altitude.offset_cm) * 0.01f
                        < g.glide_slope_min) {
            target_altitude.offset_cm = 0;
        }
    }
...
float Plane::height_above_target(void) {
    ...
    return (adjusted_altitude_cm() * 0.01f - ahrs.get_home().alt * 0.01f)
            - target_alt;
}
...
void Plane::rangefinder_height_update(void) {
    ...
    if ((rangefinder.status_orient(ROTATION_PITCH_270)
            == RangeFinder::RangeFinder_Good) && home_is_set != HOME_UNSET) {
        ...
        rangefinder_state.height_estimate = distance
                * ahrs.get_rotation_body_to_ned().c.z;
        ...
        if (rangefinder_state.in_range_count < 10) {
            if (!is_equal(distance, rangefinder_state.last_distance)
                    && fabsf(rangefinder_state.initial_range - distance)
                            > 0.05f
                                    * rangefinder.max_distance_cm_orient(
                                            ROTATION_PITCH_270) * 0.01f) {
                rangefinder_state.in_range_count++;
            }
            if (fabsf(rangefinder_state.last_distance - distance)
                    > rangefinder.max_distance_cm_orient(ROTATION_PITCH_270)
                            * 0.01 * 0.2) {
            ...
            if (!rangefinder_state.in_use
                    && (flight_stage == AP_Vehicle::FixedWing::FLIGHT_LAND
                            || control_mode == QLAND || control_mode == QRTL
                            || (control_mode == AUTO
                ...
                gcs().send_text(MAV_SEVERITY_INFO,
                        "Rangefinder engaged at %.2fm",
                        (double) rangefinder_state.height_estimate);
            }
        }
    ...
    if (rangefinder_state.in_range) {
        ...
        float correction = relative_altitude
        ...
        if (now - rangefinder_state.last_correction_time_ms > 5000) {
            ...
            rangefinder_state.correction = 0.8f * rangefinder_state.correction
                    + 0.2f * correction;
            rangefinder_state.last_correction_time_ms = now;
            if (fabsf(
                    rangefinder_state.correction
                            - rangefinder_state.initial_correction) > 30) {
                // the correction has changed by more than 30m, reset use of Lidar. We may have a bad lidar
                if (rangefinder_state.in_use) {
                    gcs().send_text(MAV_SEVERITY_INFO,
                            "Rangefinder disengaged at %.2fm",
                            (double) rangefinder_state.height_estimate);
                }
