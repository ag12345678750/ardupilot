...
// variables from parent vehicle
        ...
        // @User: Advanced
                AP_GROUPINFO("RUDDER", 3, AP_Arming_Plane, rudder_arming_value,
...
bool AP_Arming_Plane::pre_arm_checks(bool report) {
    ...
    if (plane.aparm.roll_limit_cd < 300) {
        if (report) {
            gcs().send_text(MAV_SEVERITY_CRITICAL,
                    "PreArm: LIM_ROLL_CD too small (%u)",
                    plane.aparm.roll_limit_cd);
        }
        ret = false;
    }
    ...
    if (plane.aparm.pitch_limit_max_cd < 300) {
        if (report) {
            gcs().send_text(MAV_SEVERITY_CRITICAL,
                    "PreArm: LIM_PITCH_MAX too small (%u)",
    ...
    if (plane.aparm.pitch_limit_min_cd > -300) {
        if (report) {
            gcs().send_text(MAV_SEVERITY_CRITICAL,
                    "PreArm: LIM_PITCH_MIN too large (%u)",
                    plane.aparm.pitch_limit_min_cd);
        }
        ret = false;
    }

    if (plane.channel_throttle->get_reverse() && plane.g.throttle_fs_enabled
            && plane.g.throttle_fs_value
                    < plane.channel_throttle->get_radio_max()) {
        if (report) {
            gcs().send_text(MAV_SEVERITY_CRITICAL,
                    "PreArm: Invalid THR_FS_VALUE for rev throttle");
        }
        ret = false;
    }

    if (plane.quadplane.available()
            && plane.scheduler.get_loop_rate_hz() < 100) {
        if (report) {
            gcs().send_text(MAV_SEVERITY_CRITICAL,
                    "PreArm: quadplane needs SCHED_LOOP_RATE > 100");
        }
        ret = false;
    }
    ...
    if ((checks_to_perform & ARMING_CHECK_ALL)
        ...
        if (!ahrs.healthy()) {
            if (report) {
                ...
                if (reason) {
                    gcs().send_text(MAV_SEVERITY_CRITICAL, "PreArm: %s",
                            reason);
                } else {
                    gcs().send_text(MAV_SEVERITY_CRITICAL,
