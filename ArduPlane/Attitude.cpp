            ...
            speed_scaler = 0.5f
                    + ((float) THROTTLE_CRUISE
                            / SRV_Channels::get_output_scaled(
                                    SRV_Channel::k_throttle) / 2.0f); // First order taylor expansion of square root
...
bool Plane::stick_mixing_enabled(void) {
    if (auto_throttle_mode && auto_navigation_mode) {
        ...
        if (g.stick_mixing != STICK_MIXING_DISABLED && geofence_stickmixing()
                && failsafe.state == FAILSAFE_NONE && !rc_failsafe_active()) {
...
void Plane::stabilize_roll(float speed_scaler) {
    ...
    SRV_Channels::set_output_scaled(SRV_Channel::k_aileron,
            rollController.get_servo_out(nav_roll_cd - ahrs.roll_sensor,
                    speed_scaler, disable_integrator));
}
...
void Plane::stabilize_pitch(float speed_scaler) {
    ...
    int32_t demanded_pitch = nav_pitch_cd + g.pitch_trim_cd
...
void Plane::stick_mix_channel(RC_Channel *channel, int16_t &servo_out) {
    ...
    ch_inf = (float) channel->get_radio_in()
            - (float) channel->get_radio_trim();
    ch_inf = fabsf(ch_inf);
...
void Plane::stabilize_stick_mixing_direct() {
    if (!stick_mixing_enabled() || control_mode == ACRO
            || control_mode == FLY_BY_WIRE_A || control_mode == AUTOTUNE
            || control_mode == FLY_BY_WIRE_B || control_mode == CRUISE
            || control_mode == QSTABILIZE || control_mode == QHOVER
            || control_mode == QLOITER || control_mode == QLAND
            || control_mode == QRTL || control_mode == TRAINING) {
        return;
    }
...
void Plane::stabilize_yaw(float speed_scaler) {
    ...
    if (landing.is_flaring()
            || (steer_state.hold_course_cd != -1
                    && steering_control.ground_steering)) {
        calc_nav_yaw_course();
...
void Plane::stabilize_acro(float speed_scaler) {
    float roll_rate = (channel_roll->get_control_in() / 4500.0f)
            * g.acro_roll_rate;
    float pitch_rate = (channel_pitch->get_control_in() / 4500.0f)
            * g.acro_pitch_rate;
    ...
    if (g.acro_locking && is_zero(roll_rate)) {
        ...
        SRV_Channels::set_output_scaled(SRV_Channel::k_aileron,
                rollController.get_servo_out(roll_error_cd, speed_scaler,
                true));
    } else {
        /*
        ...
        SRV_Channels::set_output_scaled(SRV_Channel::k_aileron,
                rollController.get_rate_out(roll_rate, speed_scaler));
    }
    ...
    if (g.acro_locking && is_zero(pitch_rate)) {
        ...
        SRV_Channels::set_output_scaled(SRV_Channel::k_elevator,
                pitchController.get_servo_out(nav_pitch_cd - ahrs.pitch_sensor,
                        speed_scaler,
                        false));
    } else {
        /*
        ...
        SRV_Channels::set_output_scaled(SRV_Channel::k_elevator,
                pitchController.get_rate_out(pitch_rate, speed_scaler));
    }
...
void Plane::stabilize() {
    ...
    if (quadplane.in_tailsitter_vtol_transition()) {
        ...
        nav_pitch_cd = constrain_float(
                (quadplane.tailsitter.transition_angle + 5) * 100, 5500, 8500), nav_roll_cd =
                0;
    }
...
void Plane::calc_nav_yaw_coordinated(float speed_scaler) {
    ...
    if ((control_mode == GUIDED || control_mode == AVOID_ADSB)
        ...
        commanded_rudder = yawController.get_servo_out(speed_scaler,
                disable_integrator);

        // add in rudder mixing from roll
        commanded_rudder += SRV_Channels::get_output_scaled(
                SRV_Channel::k_aileron) * g.kff_rudder_mix;
...
void Plane::calc_nav_yaw_course(void) {
    ...
    steering_control.steering = steerController.get_steering_out_angle_error(
            bearing_error_cd);
    if (stick_mixing_enabled()) {
        stick_mix_channel(channel_rudder, steering_control.steering);
    }
    steering_control.steering = constrain_int16(steering_control.steering,
            -4500, 4500);
}
...
void Plane::calc_nav_yaw_ground(void) {
    if (gps.ground_speed() < 1 && channel_throttle->get_control_in() == 0
    ...
    if (!steer_state.locked_course) {
        ...
        steering_control.steering = steerController.get_steering_out_rate(
                steer_rate);
    } else {
        // use a error controller on the summed error
        int32_t yaw_error_cd = -ToDeg(steer_state.locked_course_err) * 100;
        steering_control.steering =
                steerController.get_steering_out_angle_error(yaw_error_cd);
    }
    steering_control.steering = constrain_int16(steering_control.steering,
            -4500, 4500);
}
...
void Plane::calc_nav_pitch() {
    ...
    nav_pitch_cd = constrain_int32(commanded_pitch, pitch_limit_min_cd,
            aparm.pitch_limit_max_cd.get());
}
...
void Plane::adjust_nav_pitch_throttle(void) {
    ...
    if (throttle >= 0 && throttle < aparm.throttle_cruise
            && flight_stage != AP_Vehicle::FixedWing::FLIGHT_VTOL) {
        float p = (aparm.throttle_cruise - throttle)
                / (float) aparm.throttle_cruise;
