...
// @User: Standard
                AP_GROUPINFO_FLAGS("ENABLE", 1, QuadPlane, enable, 0,
                        AP_PARAM_FLAG_ENABLE),

        // @Group: M_
        // @Path: ../libraries/AP_Motors/AP_MotorsMulticopter.cpp
                AP_SUBGROUPVARPTR(motors, "M_", 2, QuadPlane,
                        plane.quadplane.motors_var_info),
...
#define FRAME_CLASS_OLD_IDX 30
        ...
        // @Values: 0:Continuous,1:Binary,2:VectoredYaw
                AP_GROUPINFO("TILT_TYPE", 47, QuadPlane, tilt.tilt_type,
                        TILT_TYPE_CONTINUOUS),
        ...
        // @Range: 5 80
                AP_GROUPINFO("TAILSIT_ANGLE", 48, QuadPlane,
                        tailsitter.transition_angle, 45),
        ...
        // @Values: 0:MultiCopterInput,1:FixedWingInput
                AP_GROUPINFO("TAILSIT_INPUT", 50, QuadPlane,
                        tailsitter.input_type, TAILSITTER_INPUT_MULTICOPTER),
        ...
        // @Values: 0:Disabled,1:Channel1,2:Channel2,3:Channel3,4:Channel4,5:Channel5,6:Channel6,7:Channel7,8:Channel8
                AP_GROUPINFO("TAILSIT_MASKCH", 52, QuadPlane,
                        tailsitter.input_mask_chan, 0),
        ...
        // @Range: 0 1
        // @Increment: 0.01
                AP_GROUPINFO("TAILSIT_VFGAIN", 53, QuadPlane,
                        tailsitter.vectored_forward_gain, 0),
        ...
        // @Range: 0 1
        // @Increment: 0.01
                AP_GROUPINFO("TAILSIT_VHGAIN", 54, QuadPlane,
                        tailsitter.vectored_hover_gain, 0.5),
        ...
        // @Range: 0 4
        // @Increment: 0.1
                AP_GROUPINFO("TAILSIT_VHPOW", 56, QuadPlane,
                        tailsitter.vectored_hover_power, 2.5),
...
static const struct defaults_struct defaults_table[] = {
        { "Q_A_RAT_RLL_P", 0.25 }, { "Q_A_RAT_RLL_I", 0.25 }, {
                "Q_A_RAT_RLL_FILT", 10.0 }, { "Q_A_RAT_PIT_P", 0.25 }, {
                "Q_A_RAT_PIT_I", 0.25 }, { "Q_A_RAT_PIT_FILT", 10.0 }, {
                "Q_M_SPOOL_TIME", 0.25 }, };
...
static const struct defaults_struct defaults_table_tailsitter[] = { {
        "KFF_RDDRMIX", 0.02 }, { "Q_A_RAT_PIT_FF", 0.2 }, { "Q_A_RAT_YAW_FF",
        0.2 }, { "Q_A_RAT_YAW_I", 0.18 }, { "LIM_PITCH_MAX", 3000 }, {
        "LIM_PITCH_MIN", -3000 }, { "MIXING_GAIN", 1.0 },
        { "RUDD_DT_GAIN", 10 }, { "Q_TRANSITION_MS", 2000 }, };
...
void QuadPlane::setup_default_channels(uint8_t num_motors) {
    for (uint8_t i = 0; i < num_motors; i++) {
        SRV_Channels::set_aux_channel_default(
...
bool QuadPlane::setup(void) {
     ...
     */
    AP_Int8 old_class;
    const AP_Param::ConversionInfo cinfo { Parameters::k_param_quadplane,
            FRAME_CLASS_OLD_IDX, AP_PARAM_INT8, nullptr };
    if (AP_Param::find_old_parameter(&cinfo, &old_class)
            && !frame_class.load()) {
        uint8_t new_value = 0;
    ...
    if (hal.util->available_memory()
            < 4096 + sizeof(*motors) + sizeof(*attitude_control)
                    + sizeof(*pos_control) + sizeof(*wp_nav)) {
    ...
    AP_Param::load_object_from_eeprom(attitude_control,
            attitude_control->var_info);
    pos_control = new AC_PosControl(*ahrs_view, inertial_nav, *motors,
            *attitude_control, p_alt_hold, p_vel_z, pid_accel_z, p_pos_xy,
            pi_vel_xy);
    if (!pos_control) {
    ...
    wp_nav = new AC_WPNav(inertial_nav, *ahrs_view, *pos_control,
            *attitude_control);
    if (!wp_nav) {
    ...
    motors->init((AP_Motors::motor_frame_class) frame_class.get(),
            (AP_Motors::motor_frame_type) frame_type.get());
    ...
    for (uint8_t i = 0; i < 8; i++) {
        SRV_Channel::Aux_servo_function_t func =
                (SRV_Channel::Aux_servo_function_t) (SRV_Channel::k_motor1 + i);
 ...
 */
void QuadPlane::setup_defaults_table(const struct defaults_struct *table,
        uint8_t count) {
...
void QuadPlane::setup_defaults(void) {
    ...
    if (motor_class == AP_Motors::MOTOR_FRAME_TAILSITTER) {
        setup_defaults_table(defaults_table_tailsitter,
                ARRAY_SIZE(defaults_table_tailsitter));
    }
...
void QuadPlane::run_esc_calibration(void) {
    ...
    case 1:
        ...
        motors->set_throttle_passthrough_for_esc_calibration(
                plane.channel_throttle->get_control_in() * 0.01f);
        break;
    case 2:
...
void QuadPlane::multicopter_attitude_rate_update(float yaw_rate_cds,
    ...
    if (in_vtol_mode() || is_tailsitter()) {
        ...
        attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(
                plane.nav_roll_cd, plane.nav_pitch_cd, yaw_rate_cds,
                smooth_gain);
    } else {
        ...
        attitude_control->input_euler_rate_roll_pitch_yaw(roll_rate * 100,
                pitch_rate * 100, yaw_rate_cds);
    }
}
...
void QuadPlane::control_stabilize(void) {
    ...
    float pilot_throttle_scaled = plane.channel_throttle->get_control_in()
            / 100.0f;
...
void QuadPlane::run_z_controller(void) {
    ...
    if (now - last_pidz_active_ms > 2000) {
        ...
        gcs().send_text(MAV_SEVERITY_INFO, "Reset alt target to %.1f",
                (double) inertial_nav.get_altitude());
        ...
        float base_throttle = constrain_float(
                motors->get_throttle() - motors->get_throttle_hover(), -1, 1)
                * 1000;
...
void QuadPlane::check_yaw_reset(void) {
    ...
    uint32_t new_ekfYawReset_ms = ahrs.getLastYawResetAngle(
            yaw_angle_change_rad);
    if (new_ekfYawReset_ms != ekfYawReset_ms) {
        attitude_control->shift_ef_yaw_target(
                degrees(yaw_angle_change_rad) * 100);
        ekfYawReset_ms = new_ekfYawReset_ms;
        gcs().send_text(MAV_SEVERITY_INFO, "EKF yaw reset %.2f",
                (double) degrees(yaw_angle_change_rad));
    }
}
...
bool QuadPlane::is_flying_vtol(void) {
    ...
    if (in_vtol_mode()
            && millis() - landing_detect.lower_limit_start_ms > 5000) {
...
float QuadPlane::landing_descent_rate_cms(float height_above_ground) {
    float ret = linear_interpolate(land_speed_cms, wp_nav->get_speed_down(),
            height_above_ground, land_final_alt, land_final_alt + 6);
    return ret;
}
...
void QuadPlane::control_loiter() {
    ...
    uint32_t now = AP_HAL::millis();
    if (now - last_pidz_init_ms < (uint32_t) transition_time_ms * 2
            && !is_tailsitter()) {
        // we limit pitch during initial transition
        float pitch_limit_cd = linear_interpolate(loiter_initial_pitch_cd,
                aparm.angle_max, now, last_pidz_init_ms,
                last_pidz_init_ms + transition_time_ms * 2);
    ...
    attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(
            plane.nav_roll_cd, plane.nav_pitch_cd, get_desired_yaw_rate_cds(),
            4.0f);

    if (plane.control_mode == QLAND) {
        float height_above_ground = plane.relative_ground_altitude(
        ...
        float descent_rate =
                (poscontrol.state == QPOS_LAND_FINAL) ?
                        land_speed_cms :
                        landing_descent_rate_cms(height_above_ground);
        pos_control->set_alt_target_from_climb_rate(-descent_rate, plane.G_Dt,
                true);
        check_land_complete();
        ...
        pos_control->set_alt_target_from_climb_rate_ff(
                get_pilot_desired_climb_rate_cms(), plane.G_Dt, false);
    }
    run_z_controller();
}
...
float QuadPlane::get_pilot_desired_climb_rate_cms(void) {
    ...
    uint16_t trim = (plane.channel_throttle->get_radio_max()
            + plane.channel_throttle->get_radio_min()) / 2;
    return pilot_velocity_z_max
            * plane.channel_throttle->pwm_to_angle_dz_trim(dead_zone, trim)
            / 100.0f;
}
...
void QuadPlane::init_throttle_wait(void) {
    if (plane.channel_throttle->get_control_in() >= 10 || plane.is_flying()) {
        throttle_wait = false;
    } else {
        throttle_wait = true;
    }
...
float QuadPlane::assist_climb_rate_cms(void) {
    ...
    if (plane.auto_throttle_mode) {
        ...
        climb_rate = plane.g.flybywire_climb_rate
                * (plane.nav_pitch_cd / (float) plane.aparm.pitch_limit_max_cd);
        climb_rate *= plane.channel_throttle->get_control_in();
    }
    climb_rate = constrain_float(climb_rate, -wp_nav->get_speed_down(),
            wp_nav->get_speed_up());
...
bool QuadPlane::assistance_needed(float aspeed) {
    ...
    if (labs(ahrs.roll_sensor)
            <= plane.aparm.roll_limit_cd + allowed_envelope_error_cd
            && ahrs.pitch_sensor
                    < plane.aparm.pitch_limit_max_cd + allowed_envelope_error_cd
            && ahrs.pitch_sensor
                    > -(allowed_envelope_error_cd
...
void QuadPlane::update_transition(void) {
    if (plane.control_mode == MANUAL || plane.control_mode == ACRO
     ...
     */
    if (have_airspeed && assistance_needed(aspeed) && !is_tailsitter()
        ...
        if (transition_state != TRANSITION_AIRSPEED_WAIT) {
            gcs().send_text(MAV_SEVERITY_INFO,
                    "Transition started airspeed %.1f", (double) aspeed);
        }
    ...
    if (transition_state < TRANSITION_TIMER) {
        ...
        plane.TECS_controller.set_pitch_max_limit(
                (transition_pitch_max + 1) * 2);
    }
    ...
    case TRANSITION_AIRSPEED_WAIT: {
        ...
        if (have_airspeed && aspeed > plane.aparm.airspeed_min
            ...
            gcs().send_text(MAV_SEVERITY_INFO,
                    "Transition airspeed reached %.1f", (double) aspeed);
        }
        assisted_flight = true;
    ...
    case TRANSITION_TIMER: {
        ...
        float transition_scale = (trans_time_ms
                - (millis() - transition_start_ms)) / trans_time_ms;
    ...
    case TRANSITION_ANGLE_WAIT_FW: {
        ...
        float transition_rate = tailsitter.transition_angle
                / float(transition_time_ms / 2);
        uint32_t dt = AP_HAL::millis() - transition_start_ms;
        plane.nav_pitch_cd = constrain_float((-transition_rate * dt) * 100,
                -8500, 0);
        plane.nav_roll_cd = 0;
        attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(
                plane.nav_roll_cd, plane.nav_pitch_cd, 0, smoothing_gain);
        attitude_control->set_throttle_out(motors->get_throttle_hover(), true,
                0);
        run_rate_controller();
        motors_output();
        break;
...
void QuadPlane::motors_output(void) {
    ...
    if (esc_calibration && AP_Notify::flags.esc_calibration
    ...
    motors->output();
    if (motors->armed()) {
        plane.DataFlash.Log_Write_Rate(plane.ahrs, *motors, *attitude_control,
                *pos_control);
...
bool QuadPlane::in_vtol_mode(void) const {
    ...
    return (plane.control_mode == QSTABILIZE || plane.control_mode == QHOVER
            || plane.control_mode == QLOITER || plane.control_mode == QLAND
            || plane.control_mode == QRTL
            || ((plane.control_mode == GUIDED
                    || plane.control_mode == AVOID_ADSB)
                    && plane.auto_state.vtol_loiter) || in_vtol_auto());
}
...
void QuadPlane::vtol_position_controller(void) {
    ...
    case QPOS_POSITION1: {
        ...
        if (poscontrol.speed_scale <= 0) {
            ...
            float speed_towards_target =
                    distance > 1 ? (diff_wp.normalized() * groundspeed) : 0;
            ...
            poscontrol.max_speed = MAX(speed_towards_target,
                    wp_nav->get_speed_xy() * 0.01);
        ...
        if (target_speed < final_speed) {
            ...
            target_speed_xy = target_speed_xy.normalized()
                    * poscontrol.max_speed;
        } else {
         ...
         */
        float pitch_limit_cd = linear_interpolate(-300,
                plane.aparm.pitch_limit_min_cd, plane.auto_state.wp_proportion,
                0, 1);
        if (plane.nav_pitch_cd < pitch_limit_cd) {
            plane.nav_pitch_cd = pitch_limit_cd;
        ...
        if (plane.auto_state.wp_proportion >= 1
            ...
            wp_nav->init_loiter_target();
            gcs().send_text(MAV_SEVERITY_INFO,
                    "VTOL position2 started v=%.1f d=%.1f",
    ...
    case QPOS_POSITION2: {
        ...
        if (plane.control_mode == QRTL || plane.control_mode == GUIDED
            ...
            if (poscontrol.slow_descent) {
                ...
                plane.auto_state.wp_proportion = location_path_proportion(
                        plane.current_loc, plane.prev_WP_loc,
                        plane.next_WP_loc);
                target_altitude = linear_interpolate(plane.prev_WP_loc.alt,
                        plane.next_WP_loc.alt, plane.auto_state.wp_proportion,
                        0, 1);
            }
    ...
    case QPOS_LAND_DESCEND: {
        float height_above_ground = plane.relative_ground_altitude(
                plane.g.rangefinder_landing);
        pos_control->set_alt_target_from_climb_rate(
                -landing_descent_rate_cms(height_above_ground), plane.G_Dt,
                true);
        break;
    }

    case QPOS_LAND_FINAL:
        pos_control->set_alt_target_from_climb_rate(-land_speed_cms, plane.G_Dt,
                true);
        break;
...
void QuadPlane::takeoff_controller(void) {
    ...
    pos_control->set_alt_target_from_climb_rate(wp_nav->get_speed_up(),
            plane.G_Dt, true);
    run_z_controller();
}
...
void QuadPlane::waypoint_controller(void) {
    ...
    attitude_control->input_euler_angle_roll_pitch_yaw(wp_nav->get_roll(),
            wp_nav->get_pitch(), wp_nav->get_yaw(),
            true, 4.0f);
    // nav roll and pitch are controller by loiter controller
    plane.nav_roll_cd = wp_nav->get_roll();
    plane.nav_pitch_cd = wp_nav->get_pitch();
    ...
    pos_control->set_alt_target_from_climb_rate_ff(assist_climb_rate_cms(),
            plane.G_Dt, false);
    run_z_controller();
}
...
bool QuadPlane::verify_vtol_takeoff(const AP_Mission::Mission_Command &cmd) {
    ...
    transition_state =
            is_tailsitter() ?
                    TRANSITION_ANGLE_WAIT_FW : TRANSITION_AIRSPEED_WAIT;
    plane.TECS_controller.set_pitch_max_limit(transition_pitch_max);
    set_alt_target_current();
...
bool QuadPlane::verify_vtol_land(void) {
    ...
    if (poscontrol.state == QPOS_LAND_DESCEND
            && height_above_ground < land_final_alt) {
        poscontrol.state = QPOS_LAND_FINAL;
...
void QuadPlane::Log_Write_QControl_Tuning() {
    ...
    struct log_QControl_Tuning pkt =
            {
            LOG_PACKET_HEADER_INIT(LOG_QTUN_MSG), time_us : AP_HAL::micros64(),
                    angle_boost : attitude_control->angle_boost(), throttle_out
                            : motors->get_throttle(), desired_alt
                            : pos_control->get_alt_target() / 100.0f, inav_alt
                            : inertial_nav.get_altitude() / 100.0f,
                    desired_climb_rate
                            : (int16_t) pos_control->get_vel_target_z(),
                    climb_rate : (int16_t) inertial_nav.get_velocity_z(), dvx
                            : desired_velocity.x * 0.01f, dvy
                            : desired_velocity.y * 0.01f, dax : accel_target.x
                            * 0.01f, day : accel_target.y * 0.01f, throttle_mix
...
int8_t QuadPlane::forward_throttle_pct(void) {
     ...
     */
    if (!in_vtol_mode() || !motors->armed() || vel_forward.gain <= 0
            || plane.control_mode == QSTABILIZE
    ...
    Vector3f vel_error_body = ahrs.get_rotation_body_to_ned().transposed()
            * ((desired_velocity_cms * 0.01f) - vel_ned);
    ...
    // WP nav speed.
    fwd_vel_error -= (wp_nav->get_speed_xy() * 0.01f) * plane.nav_pitch_cd
            / (float) plane.aparm.pitch_limit_max_cd;
    ...
    int8_t fwd_throttle_min =
            (plane.aparm.throttle_min <= 0) ? 0 : plane.aparm.throttle_min;
    vel_forward.integrator = constrain_float(vel_forward.integrator,
            fwd_throttle_min, plane.aparm.throttle_max);
...
float QuadPlane::get_weathervane_yaw_rate_cds(void) {
     ...
     */
    if (!in_vtol_mode() || !motors->armed() || weathervane.gain <= 0
            || plane.control_mode == QSTABILIZE
            || plane.control_mode == QHOVER) {
...
void QuadPlane::adjust_alt_target(float altitude_cm) {
    ...
    float target_cm = constrain_float(altitude_cm, current_alt - 50,
            current_alt + 50);
...
bool QuadPlane::is_vtol_takeoff(uint16_t id) const {
    ...
    if (id == MAV_CMD_NAV_TAKEOFF && available()
            && (options & OPTION_ALLOW_FW_TAKEOFF) == 0) {
...
bool QuadPlane::is_vtol_land(uint16_t id) const {
    ...
    if (id == MAV_CMD_NAV_LAND && available()
            && (options & OPTION_ALLOW_FW_LAND) == 0) {
...
float QuadPlane::stopping_distance(void) {
    ...
    return plane.ahrs.groundspeed_vector().length_squared()
            / (2 * transition_decel);
}
