...
void QuadPlane::tailsitter_output(void) {
    ...
    if (!tailsitter_active() || in_tailsitter_vtol_transition()) {
        if (tailsitter.vectored_forward_gain > 0) {
            ...
            float aileron = SRV_Channels::get_output_scaled(
                    SRV_Channel::k_aileron);
            float elevator = SRV_Channels::get_output_scaled(
                    SRV_Channel::k_elevator);
            float tilt_left = (elevator + aileron)
                    * tailsitter.vectored_forward_gain;
            float tilt_right = (elevator - aileron)
                    * tailsitter.vectored_forward_gain;
    ...
    if (tailsitter.vectored_hover_gain > 0) {
         ...
         */
        int32_t pitch_error_cd = (plane.nav_pitch_cd - ahrs_view->pitch_sensor)
                * 0.5;
        float extra_pitch = constrain_float(pitch_error_cd, -4500, 4500)
                / 4500.0;
        float extra_sign = extra_pitch > 0 ? 1 : -1;
        float extra_elevator = extra_sign
                * powf(fabsf(extra_pitch), tailsitter.vectored_hover_power)
                * 4500;
        float tilt_left = extra_elevator
                + (elevator + aileron) * tailsitter.vectored_hover_gain;
        float tilt_right = extra_elevator
                + (elevator - aileron) * tailsitter.vectored_hover_gain;
    ...
    if (tailsitter.input_mask_chan > 0 && tailsitter.input_mask > 0
        ...
        if (tailsitter.input_mask & TAILSITTER_MASK_THROTTLE) {
            SRV_Channels::set_output_scaled(SRV_Channel::k_throttle,
                    plane.channel_throttle->get_control_in_zero_dz());
        }
...
bool QuadPlane::tailsitter_transition_fw_complete(void) {
    ...
    if (labs(ahrs_view->pitch_sensor) > tailsitter.transition_angle * 100
            ...
            || AP_HAL::millis() - transition_start_ms
...
void QuadPlane::tailsitter_speed_scaling(void) {
    ...
    if (is_zero(throttle)) {
        ...
        scaling = constrain_float(hover_throttle / throttle, 1 / scaling_max,
                scaling_max);
    }
