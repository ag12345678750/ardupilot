...
bool Plane::suppress_throttle(void) {
    ...
    bool gps_movement = (gps.status() >= AP_GPS::GPS_OK_FIX_2D
            && gps.ground_speed() >= 5);

    if (control_mode == AUTO && auto_state.takeoff_complete == false) {

        uint32_t launch_duration_ms = ((int32_t) g.takeoff_throttle_delay) * 100
                + 2000;
        if (is_flying()
                && millis() - started_flying_ms > MAX(launch_duration_ms, 5000U)
 ...
 */
void Plane::channel_output_mixer_pwm(uint8_t mixing_type, uint16_t &chan1_out,
        uint16_t &chan2_out) const {
    int16_t c1, c2;
    int16_t v1, v2;
 ...
 */
void Plane::channel_output_mixer(uint8_t mixing_type,
        SRV_Channel::Aux_servo_function_t func1,
        SRV_Channel::Aux_servo_function_t func2) {
    SRV_Channel *chan1, *chan2;
    if (!(chan1 = SRV_Channels::get_channel_for(func1)) || !(chan2 =
            SRV_Channels::get_channel_for(func2))) {
        return;
    }
...
/*
 ...
 SERVOn_* parameters
 */
void Plane::channel_function_mixer(SRV_Channel::Aux_servo_function_t func1_in,
        SRV_Channel::Aux_servo_function_t func2_in,
        SRV_Channel::Aux_servo_function_t func1_out,
        SRV_Channel::Aux_servo_function_t func2_out) {
...
void Plane::flaperon_update(int8_t flap_percent) {
    ...
    float flaperon_left = constrain_float(aileron + flap_percent * 45, -4500,
            4500);
    float flaperon_right = constrain_float(aileron - flap_percent * 45, -4500,
            4500);
    SRV_Channels::set_output_scaled(SRV_Channel::k_flaperon_left,
            flaperon_left);
    SRV_Channels::set_output_scaled(SRV_Channel::k_flaperon_right,
            flaperon_right);
}
...
void Plane::dspoiler_update(void) {
    ...
    float elevon_left = SRV_Channels::get_output_scaled(
            SRV_Channel::k_elevon_left);
    float elevon_right = SRV_Channels::get_output_scaled(
    ...
    SRV_Channels::set_output_scaled(SRV_Channel::k_dspoilerLeft1,
            dspoiler1_left);
    SRV_Channels::set_output_scaled(SRV_Channel::k_dspoilerLeft2,
            dspoiler2_left);
    SRV_Channels::set_output_scaled(SRV_Channel::k_dspoilerRight1,
            dspoiler1_right);
    SRV_Channels::set_output_scaled(SRV_Channel::k_dspoilerRight2,
            dspoiler2_right);
}
...
void Plane::set_servos_manual_passthrough(void) {
    ...
    SRV_Channels::set_output_scaled(SRV_Channel::k_throttle,
            channel_throttle->get_control_in_zero_dz());
}
...
void Plane::throttle_watt_limiter(int8_t &min_throttle, int8_t &max_throttle) {
    ...
    if (battery.overpower_detected()) {
        ...
        if (SRV_Channels::get_output_scaled(SRV_Channel::k_throttle)
                > throttle_watt_limit_max && // demanding max forward thrust
                throttle_watt_limit_max > 0) { // and we're currently limiting it
            throttle_watt_limit_timer_ms = now;
            throttle_watt_limit_max--;

        } else if (SRV_Channels::get_output_scaled(SRV_Channel::k_throttle)
                < throttle_watt_limit_min && // demanding max negative thrust
    ...
    max_throttle = constrain_int16(max_throttle, 0,
            max_throttle - throttle_watt_limit_max);
    if (min_throttle < 0) {
        min_throttle = constrain_int16(min_throttle,
                min_throttle + throttle_watt_limit_min, 0);
    }
}
    ...
    if (!hal.util->get_soft_armed()) {
        if (arming.arming_required() == AP_Arming::YES_ZERO_PWM) {
            SRV_Channels::set_output_limit(SRV_Channel::k_throttle,
                    SRV_Channel::SRV_CHANNEL_LIMIT_ZERO_PWM);
        } else {
        ...
        if (g.throttle_suppress_manual) {
            ...
            && (control_mode == STABILIZE || control_mode == TRAINING
                    || control_mode == ACRO || control_mode == FLY_BY_WIRE_A
                    || control_mode == AUTOTUNE) && !failsafe.ch3_counter) {
...
void Plane::set_servos_flaps(void) {
    ...
    if (flapin != nullptr && !failsafe.ch3_failsafe
            && failsafe.ch3_counter == 0) {
        flapin->input();
    ...
    SRV_Channels::set_output_scaled(SRV_Channel::k_flap_auto,
            auto_flap_percent);
...
void Plane::servo_output_mixers(void) {
    ...
    channel_function_mixer(SRV_Channel::k_rudder, SRV_Channel::k_elevator,
            SRV_Channel::k_vtail_right, SRV_Channel::k_vtail_left);
...
void Plane::set_servos(void) {
    ...
    if (control_mode >= FLY_BY_WIRE_B || quadplane.in_assisted_flight()
    ...
    if (landing.get_then_servos_neutral() > 0 && control_mode == AUTO
            && landing.get_disarm_delay() > 0 && landing.is_complete()
            && !arming.is_armed()) {
        ...
        if (landing.get_then_servos_neutral() == 1) {
            ...
            SRV_Channels::set_output_limit(SRV_Channel::k_aileron,
                    SRV_Channel::SRV_CHANNEL_LIMIT_ZERO_PWM);
            SRV_Channels::set_output_limit(SRV_Channel::k_elevator,
                    SRV_Channel::SRV_CHANNEL_LIMIT_ZERO_PWM);
            SRV_Channels::set_output_limit(SRV_Channel::k_rudder,
                    SRV_Channel::SRV_CHANNEL_LIMIT_ZERO_PWM);
        }
    }
...
void Plane::servos_auto_trim(void) {
    ...
    g2.servo_channels.adjust_trim(SRV_Channel::k_elevon_right,
            pitch_I + roll_I);
    ...
    g2.servo_channels.adjust_trim(SRV_Channel::k_dspoilerLeft1,
            pitch_I - roll_I);
    g2.servo_channels.adjust_trim(SRV_Channel::k_dspoilerLeft2,
            pitch_I - roll_I);
    g2.servo_channels.adjust_trim(SRV_Channel::k_dspoilerRight1,
            pitch_I + roll_I);
    g2.servo_channels.adjust_trim(SRV_Channel::k_dspoilerRight2,
            pitch_I + roll_I);
