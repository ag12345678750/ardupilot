...
void QuadPlane::tiltrotor_slew(float newtilt) {
    ...
    tilt.current_tilt = constrain_float(newtilt, tilt.current_tilt - max_change,
            tilt.current_tilt + max_change);

    // translate to 0..1000 range and output
    SRV_Channels::set_output_scaled(SRV_Channel::k_motor_tilt,
            1000 * tilt.current_tilt);

    // setup tilt compensation
    motors->set_thrust_compensation_callback(
            FUNCTOR_BIND_MEMBER(&QuadPlane::tilt_compensate, void, float*,
                    uint8_t));
}
...
void QuadPlane::tiltrotor_continuous_update(void) {
    ...
    if (!in_vtol_mode() && (!hal.util->get_soft_armed() || !assisted_flight)) {
        ...
        float new_throttle = constrain_float(
                SRV_Channels::get_output_scaled(SRV_Channel::k_throttle) * 0.01,
                0, 1);
        ...
        if (!hal.util->get_soft_armed()) {
            ...
            uint8_t mask =
                    is_zero(tilt.current_throttle) ?
                            0 : (uint8_t) tilt.tilt_mask.get();
    ...
    if (assisted_flight && transition_state >= TRANSITION_TIMER) {
...
void QuadPlane::tiltrotor_binary_slew(bool forward) {
    ...
    if (forward) {
        tilt.current_tilt = constrain_float(tilt.current_tilt + max_change, 0,
                1);
    } else {
        tilt.current_tilt = constrain_float(tilt.current_tilt - max_change, 0,
                1);
...
void QuadPlane::tiltrotor_binary_update(void) {
    ...
    if (!in_vtol_mode()) {
        ...
        float new_throttle = SRV_Channels::get_output_scaled(
                SRV_Channel::k_throttle) * 0.01f;
        if (tilt.current_tilt >= 1) {
            uint8_t mask =
                    is_zero(new_throttle) ? 0 : (uint8_t) tilt.tilt_mask.get();
            // the motors are all the way forward, start using them for fwd thrust
...
void QuadPlane::tiltrotor_vectored_yaw(void) {
    ...
    if (no_yaw) {
        ...
        SRV_Channels::set_output_scaled(SRV_Channel::k_tiltMotorLeft,
                1000 * (base_output + yaw_out * yaw_range));
        SRV_Channels::set_output_scaled(SRV_Channel::k_tiltMotorRight,
                1000 * (base_output - yaw_out * yaw_range));
    }
}
