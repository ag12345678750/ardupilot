...
// Units:   Hz      us
        ...
        SCHED_TASK(read_rangefinder, 50, 100),   //alex it was 50
...
void Plane::loop() {
    ...
    if (perf.delta_us_fast_loop > perf.G_Dt_max
            && perf.fast_loopTimer_us != 0) {
...
void Plane::afs_fs_check(void) {
    ...
    afs.check(failsafe.last_heartbeat_ms, geofence_breached(),
...
void Plane::log_perf_info() {
    if (scheduler.debug() != 0) {
        gcs().send_text(MAV_SEVERITY_INFO, "PERF: %u/%u Dt=%u/%u Log=%u",
                (unsigned) perf.num_long, (unsigned) perf.mainLoop_count,
                (unsigned) perf.G_Dt_max, (unsigned) perf.G_Dt_min,
...
void Plane::compass_save() {
    if (g.compass_enabled && compass.get_learn_type() >= Compass::LEARN_INTERNAL
...
void Plane::airspeed_ratio_update(void) {
    if (!airspeed.enabled() || gps.status() < AP_GPS::GPS_OK_FIX_3D
            || gps.ground_speed() < 4) {
...
void Plane::handle_auto_mode(void) {
    ...
    if (mission.state() != AP_Mission::MISSION_RUNNING) {
        ...
        gcs().send_text(MAV_SEVERITY_INFO,
                "Aircraft in auto without a running mission");
        return;
    }
...
void Plane::update_flight_mode(void) {
    ...
    case FLY_BY_WIRE_A: {
        ...
        nav_roll_cd = constrain_int32(nav_roll_cd, -roll_limit_cd,
                roll_limit_cd);
        update_load_factor();
        ...
        if (failsafe.ch3_failsafe && g.short_fs_action == 2) {
            ...
            SRV_Channels::set_output_limit(SRV_Channel::k_throttle,
                    SRV_Channel::SRV_CHANNEL_LIMIT_MIN);
        }
    ...
    case QRTL: {
        ...
        if (quadplane.tailsitter_active()) {
            ...
            if (pitch_input > 0) {
                nav_pitch_cd = pitch_input
                        * MIN(aparm.pitch_limit_max_cd,
                                quadplane.aparm.angle_max);
            } else {
                nav_pitch_cd = pitch_input
                        * MIN(-pitch_limit_min_cd, quadplane.aparm.angle_max);
            }
            nav_pitch_cd = constrain_int32(nav_pitch_cd, pitch_limit_min_cd,
                    aparm.pitch_limit_max_cd.get());
        }
    ...
    case RTL:
        if (quadplane.available() && quadplane.rtl_mode == 1
                && (nav_controller->reached_loiter_target()
                        || location_passed_point(current_loc, prev_WP_loc,
                                next_WP_loc)
                        || auto_state.wp_distance
                                < MAX(qrtl_radius,
                                        quadplane.stopping_distance()))
            ...
            break;
        } else if (g.rtl_autoland == 1 && !auto_state.checked_for_autoland
                && reached_loiter_target() && labs(altitude_error_cm) < 1000) {
        ...
        } else if (g.rtl_autoland == 2 && !auto_state.checked_for_autoland) {
...
void Plane::set_flight_stage(AP_Vehicle::FixedWing::FlightStage fs) {
    ...
    landing.handle_flight_stage_change(
            fs == AP_Vehicle::FixedWing::FLIGHT_LAND);
...
void Plane::update_alt() {
    ...
    if (auto_throttle_mode && !throttle_suppressed) {
        ...
        SpdHgt_Controller->update_pitch_throttle(relative_target_altitude_cm(),
                target_airspeed_cm, flight_stage, distance_beyond_land_wp,
                get_takeoff_pitch_min_cd(), throttle_nudge, tecs_hgt_afe(),
...
void Plane::update_flight_stage(void) {
    ...
    if (auto_throttle_mode && !throttle_suppressed) {
        if (control_mode == AUTO) {
            if (quadplane.in_vtol_auto()) {
                ...
                if (landing.is_commanded_go_around()
                        || flight_stage
    ...
    } else if (quadplane.in_vtol_mode() || quadplane.in_assisted_flight()) {
        set_flight_stage(AP_Vehicle::FixedWing::FLIGHT_VTOL);
    } else {
        set_flight_stage(AP_Vehicle::FixedWing::FLIGHT_NORMAL);
    }
...
void Plane::disarm_if_autoland_complete() {
    if (landing.get_disarm_delay() > 0 && !is_flying()
            && arming.arming_required() != AP_Arming::NO && arming.is_armed()) {
        /* we have auto disarm enabled. See if enough time has passed */
        if (millis() - auto_state.last_flying_ms
                >= landing.get_disarm_delay() * 1000UL) {
            if (disarm_motors()) {
