...
void Plane::geofence_load(void) {
    ...
    if (geofence_state == nullptr) {
        ...
        if (hal.util->available_memory()
                < 100 + boundary_size + sizeof(struct GeofenceState)) {
            // too risky to enable as we could run out of stack
            goto failed;
        }
        geofence_state = (struct GeofenceState*) calloc(1,
                sizeof(struct GeofenceState));
        if (geofence_state == nullptr) {
    ...
    if (!Polygon_complete(&geofence_state->boundary[1],
            geofence_state->num_points - 1)) {
        // first point and last point must be the same
        goto failed;
    }
    if (Polygon_outside(geofence_state->boundary[0],
            &geofence_state->boundary[1], geofence_state->num_points - 1)) {
    ...
    failed: g.fence_action.set(FENCE_ACTION_NONE);
...
bool Plane::geofence_present(void) {
    ...
    if (g.fence_action
            == FENCE_ACTION_NONE|| g.fence_total < MIN_GEOFENCE_POINTS) {
        return false;
    }
    return true;
}
...
void Plane::geofence_update_pwm_enabled_state() {
    ...
    if (geofence_state->is_pwm_enabled
            != geofence_state->previous_is_pwm_enabled) {
...
bool Plane::geofence_enabled(void) {
    ...
    if (g.fence_action == FENCE_ACTION_NONE || !geofence_present()
...
void Plane::geofence_check(bool altitude_check_only) {
    if (!geofence_enabled()) {
        ...
        if (geofence_state != nullptr
                ...
                && geofence_present() && geofence_state->boundary_uptodate
    ...
    if (geofence_state->floor_enabled && geofence_check_minalt()) {
        ...
        location.x = loc.lat;
        location.y = loc.lng;
        outside = Polygon_outside(location, &geofence_state->boundary[1],
                geofence_state->num_points - 1);
        if (outside) {
    ...
    case FENCE_ACTION_RTL:
        ...
        if (g.fence_ret_rally != 0 || g.fence_action == FENCE_ACTION_RTL) { //return to a rally point
            guided_WP_loc = rally.calc_best_rally_or_home_location(current_loc,
                    get_RTL_altitude());
            ...
            if (g.fence_retalt > 0) {
                ...
                guided_WP_loc.alt = home.alt
                        + 100.0f * (g.fence_minalt + g.fence_maxalt) / 2;
            }
...
void Plane::geofence_send_status(mavlink_channel_t chan) {
    if (geofence_enabled() && geofence_state != nullptr) {
        mavlink_msg_fence_status_send(chan,
                ...
                geofence_state->breach_count, geofence_state->breach_type,
