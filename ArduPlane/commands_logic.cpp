...
bool Plane::start_command(const AP_Mission::Mission_Command &cmd) {
    ...
    if (AP_Mission::is_nav_cmd(cmd)) {
        ...
        auto_state.wp_is_land_approach = mission.get_next_nav_cmd(next_index,
                next_nav_cmd) && (next_nav_cmd.id == MAV_CMD_NAV_LAND)
                && !quadplane.is_vtol_land(next_nav_cmd.id);

        gcs().send_text(MAV_SEVERITY_INFO, "Executing nav command ID #%i",
                cmd.id);
    } else {
    ...
    case MAV_CMD_DO_SET_SERVO:
        ServoRelayEvents.do_set_servo(cmd.content.servo.channel,
                cmd.content.servo.pwm);
        break;

    case MAV_CMD_DO_SET_RELAY:
        ServoRelayEvents.do_set_relay(cmd.content.relay.num,
                cmd.content.relay.state);
        break;

    case MAV_CMD_DO_REPEAT_SERVO:
        ServoRelayEvents.do_repeat_servo(cmd.content.repeat_servo.channel,
    ...
    case MAV_CMD_DO_REPEAT_RELAY:
        ServoRelayEvents.do_repeat_relay(cmd.content.repeat_relay.num,
        ...
        if (cmd.p1 != 2) {
            if (!geofence_set_enabled((bool) cmd.p1, AUTO_TOGGLED)) {
                gcs().send_text(MAV_SEVERITY_WARNING,
                        "Unable to set fence. Enabled state to %u", cmd.p1);
            } else {
                gcs().send_text(MAV_SEVERITY_INFO,
                        "Set fence enabled state to %u", cmd.p1);
            }
        } else { //commanding to only disable floor
            if (!geofence_set_floor_enabled(false)) {
                gcs().send_text(MAV_SEVERITY_WARNING,
                        "Unable to disable fence floor");
            } else {
    ...
    case MAV_CMD_DO_MOUNT_CONTROL:          // 205
        ...
        camera_mount.set_angle_targets(cmd.content.mount_control.roll,
                cmd.content.mount_control.pitch, cmd.content.mount_control.yaw);
        break;
#endif

    case MAV_CMD_DO_VTOL_TRANSITION:
        plane.quadplane.handle_do_vtol_transition(
                (enum MAV_VTOL_STATE) cmd.content.do_vtol_transition.target_state);
        break;

    case MAV_CMD_DO_ENGINE_CONTROL:
        plane.g2.ice_control.engine_control(
                cmd.content.do_engine_control.start_control,
...
bool Plane::verify_command(const AP_Mission::Mission_Command &cmd) // Returns true if command complete
    ...
    case MAV_CMD_NAV_LAND:
        ...
        if (flight_stage
                == AP_Vehicle::FixedWing::FlightStage::FLIGHT_ABORT_LAND) {
            return landing.verify_abort_landing(prev_WP_loc, next_WP_loc,
                    current_loc, auto_state.takeoff_altitude_rel_cm,
                    throttle_suppressed);
            ...
            return landing.verify_land(prev_WP_loc, next_WP_loc, current_loc,
                    height, auto_state.sink_rate, auto_state.wp_proportion,
                    auto_state.last_flying_ms, arming.is_armed(), is_flying(),
...
void Plane::do_RTL(int32_t rtl_altitude) {
    ...
    next_WP_loc = rally.calc_best_rally_or_home_location(current_loc,
            rtl_altitude);
    ...
    if (g.fence_autoenable == 1) {
        if (!geofence_set_enabled(false, AUTO_TOGGLED)) {
            gcs().send_text(MAV_SEVERITY_NOTICE,
                    "Disable fence failed (autodisable)");
        } else {
            gcs().send_text(MAV_SEVERITY_NOTICE,
                    "Fence disabled (autodisable)");
        }
    } else if (g.fence_autoenable == 2) {
        if (!geofence_set_floor_enabled(false)) {
            gcs().send_text(MAV_SEVERITY_NOTICE,
                    "Disable fence floor failed (autodisable)");
        } else {
            gcs().send_text(MAV_SEVERITY_NOTICE,
                    "Fence floor disabled (auto disable)");
        }
    }
#endif
}
...
bool Plane::verify_takeoff() {
    if (ahrs.yaw_initialised() && steer_state.hold_course_cd == -1) {
        ...
        if (auto_state.takeoff_speed_time_ms != 0
            ...
            float takeoff_course = wrap_PI(
                    radians(gps.ground_course_cd() * 0.01f))
                    - steer_state.locked_course_err;
            takeoff_course = wrap_PI(takeoff_course);
            steer_state.hold_course_cd = wrap_360_cd(
                    degrees(takeoff_course) * 100);
            gcs().send_text(MAV_SEVERITY_INFO,
                    "Holding course %ld at %.1fm/s (%.1f)",
                    steer_state.hold_course_cd, (double) gps.ground_speed(),
...
bool Plane::verify_nav_wp(const AP_Mission::Mission_Command &cmd) {
    ...
    if (cmd_passby > 0) {
        ...
        if (!is_zero(dist)) {
            ...
            flex_next_WP_loc.lat = flex_next_WP_loc.lat
                    + (flex_next_WP_loc.lat - prev_WP_loc.lat)
                            * (factor - 1.0f);
            flex_next_WP_loc.lng = flex_next_WP_loc.lng
                    + (flex_next_WP_loc.lng - prev_WP_loc.lng)
                            * (factor - 1.0f);
        }
    }
    ...
    if (cmd_acceptance_distance > 0) {
        ...
        acceptance_distance_m = nav_controller->turn_distance(g.waypoint_radius,
...
bool Plane::verify_loiter_to_alt() {
    ...
    if (condition_value == 0) {
        ...
        if (labs(loiter.sum_cd) > 1
                && (loiter.reached_target_alt
                        || loiter.unable_to_acheive_target_alt)) {
            // primary goal completed, initialize secondary heading goal
            if (loiter.unable_to_acheive_target_alt) {
                gcs().send_text(MAV_SEVERITY_INFO,
                        "Loiter to alt was stuck at %d", current_loc.alt / 100);
            }
...
bool Plane::verify_continue_and_change_alt() {
    ...
    if (locations_are_same(prev_WP_loc, next_WP_loc)
        ...
        if (get_distance(current_loc, next_WP_loc) < 200.0f) {
            ...
            int32_t next_wp_bearing_cd = get_bearing_cd(prev_WP_loc,
                    next_WP_loc);
...
bool Plane::verify_altitude_wait(const AP_Mission::Mission_Command &cmd) {
    ...
    if (auto_state.sink_rate > cmd.content.altitude_wait.descent_rate) {
        gcs().send_text(MAV_SEVERITY_INFO, "Reached descent rate %.1f m/s",
    ...
    if (cmd.content.altitude_wait.wiggle_time != 0) {
        ...
        if (auto_state.idle_wiggle_stage == 0
                && AP_HAL::millis() - last_wiggle_ms
                        > cmd.content.altitude_wait.wiggle_time * 1000) {
...
void Plane::do_change_speed(const AP_Mission::Mission_Command &cmd) {
    ...
    case 0:             // Airspeed
        if (cmd.content.speed.target_ms > 0) {
            ...
            gcs().send_text(MAV_SEVERITY_INFO, "Set airspeed %u m/s",
                    (unsigned) cmd.content.speed.target_ms);
        }
        break;
    case 1:             // Ground speed
        gcs().send_text(MAV_SEVERITY_INFO, "Set groundspeed %u",
                (unsigned) cmd.content.speed.target_ms);
    ...
    if (cmd.content.speed.throttle_pct > 0
            && cmd.content.speed.throttle_pct <= 100) {
        gcs().send_text(MAV_SEVERITY_INFO, "Set throttle %u",
                (unsigned) cmd.content.speed.throttle_pct);
...
bool Plane::verify_loiter_heading(bool init) {
    ...
    if (get_distance(next_WP_loc, next_nav_cmd.content.location)
            < abs(aparm.loiter_radius)) {
    ...
    // Bearing in degrees
    int32_t bearing_cd = get_bearing_cd(current_loc,
            next_nav_cmd.content.location);
    ...
    /*
     ...
     particular moment.
     */
    if (labs(heading_err_cd) <= 1000 || loiter.sum_cd > loiter.total_cd) {
