...
void Plane::set_control_channels(void) {
    ...
    if (!arming.is_armed()
            && arming.arming_required() == AP_Arming::YES_MIN_PWM) {
        SRV_Channels::set_safety_limit(SRV_Channel::k_throttle,
                aparm.throttle_min < 0 ?
...
void Plane::rudder_arm_disarm_check() {
    ...
    if (!arming.is_armed()) {
        ...
        if (channel_rudder->get_control_in() > 4000) {
            ...
            if (rudder_arm_timer == 0 || now - rudder_arm_timer < 3000) {
    ...
    } else if (arming_rudder == AP_Arming_Plane::ARMING_RUDDER_ARMDISARM
            && !is_flying()) {
...
void Plane::read_radio() {
    ...
    if (g.throttle_nudge
        ...
        float nudge = (SRV_Channels::get_output_scaled(SRV_Channel::k_throttle)
                - 50) * 0.02f;
        if (ahrs.airspeed_sensor_enabled()) {
            airspeed_nudge_cm = (aparm.airspeed_max * 100
                    - aparm.airspeed_cruise_cm) * nudge;
        } else {
            throttle_nudge = (aparm.throttle_max - aparm.throttle_cruise)
                    * nudge;
        }
    } else {
        airspeed_nudge_cm = 0;
    ...
    if (g.throttle_fs_enabled) {
        if (rc_failsafe_active()) {
            ...
            if (failsafe.ch3_counter == 10) {
                gcs().send_text(MAV_SEVERITY_WARNING, "Throttle failsafe on %u",
                        (unsigned) pwm);
            ...
            if (failsafe.ch3_counter == 1) {
                gcs().send_text(MAV_SEVERITY_WARNING,
                        "Throttle failsafe off %u", (unsigned) pwm);
            } else if (failsafe.ch3_counter == 0) {
...
void Plane::trim_control_surfaces() {
    read_radio();
    int16_t trim_roll_range = (channel_roll->get_radio_max()
            - channel_roll->get_radio_min()) / 5;
    int16_t trim_pitch_range = (channel_pitch->get_radio_max()
            - channel_pitch->get_radio_min()) / 5;
    if (channel_roll->get_radio_in()
            < channel_roll->get_radio_min() + trim_roll_range
            || channel_roll->get_radio_in()
                    > channel_roll->get_radio_max() - trim_roll_range
            || channel_pitch->get_radio_in()
                    < channel_pitch->get_radio_min() + trim_pitch_range
            || channel_pitch->get_radio_in()
                    > channel_pitch->get_radio_max() - trim_pitch_range) {
